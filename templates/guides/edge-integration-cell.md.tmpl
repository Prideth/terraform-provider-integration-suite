...
## Access Policy replication: no confirmed write path

Edge Integration Cell's own Access Policies UI lets an administrator "select the runtimes where
you want to create the new access policy" and later edit that selection — implying access
policies can be scoped to specific Edge nodes. The `AccessPolicies` OData entity this provider
already manages as `sapintegrationsuite_access_policy` exposes no writable runtime/location field
anywhere in its confirmed schema, only a computed `ReconciliationStatus` reporting replication
state after the fact. This provider does not automate the runtime-selection step through the UI,
and continues to expose only the read-only reconciliation status it already confirmed.
`edge_integration_cell.access_policy_replication` in `docs/feature-support.md` records this.
