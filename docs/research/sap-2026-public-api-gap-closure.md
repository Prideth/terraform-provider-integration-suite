...
|---|---|---|
| Access policies | Supported, lifecycle verified on a tenant | Property names from SAP's own access policy tooling and the `$metadata`; `Edm.Int64` keys; `PUT` for updates; runtime assignments data source; create, `$filter` lookup, read, reference create and delete verified on a tenant; description changes use `PATCH`, because the tenant rejects `PUT` with `501` |
