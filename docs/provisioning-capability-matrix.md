...
| API Gateway / API Artifacts capability | Unsupported – manual bootstrap required (UI-driven capability activation) | Unsupported (capability-level; individual API Artifacts have their own design-time API, tracked separately, planned v0.2.x) | Unsupported | Unsupported | Unsupported | N/A | Unsupported |
