...
func TestClient_UndeployIntegrationFlow(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if err := client.UndeployIntegrationFlow(context.Background(), "metering"); err != nil {
		t.Fatalf("UndeployIntegrationFlow() error: %v", err)
	}
}
